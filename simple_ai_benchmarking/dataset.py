...
def _is_memory_sufficient_for_arrays(*arrays_info) -> bool:
    ...
    """
    required_memory = sum(_estimate_array_memory_usage(shape, dtype) for shape, dtype in arrays_info)
...
class SyntheticDataset(Dataset):
    ...
    def prepare(self) -> None:
        ...
        if _is_memory_sufficient_for_arrays(
            ...
            logger.debug(
                "Synthetic Dataset Inputs Shape & Type: {} {}", self.inputs.shape, self.inputs.dtype
            )
            logger.debug(
                "Synthetic Dataset Targets Shape & Type: {} {}", self.targets.shape, self.targets.dtype
            )
        else:
            raise MemoryError("Insufficient memory, select lower num_batches or batch_size.")
...
class SyntheticPytorchDataset(SyntheticDataset):
    ...
    def _create_data(self) -> None:
        ...
        self.targets = torch.randint(low=0, high=2, size=self.dataset_targets_shape, dtype=torch.int64).cpu()
...
class SyntheticTensorFlowDataset(SyntheticDataset):
    ...
    def _create_data(self):
        ...
        self.targets = tf.random.uniform(self.dataset_targets_shape, minval=0, maxval=2, dtype=tf.int64)
